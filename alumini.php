...
$row = mysqli_fetch_assoc($result);
		...
		if ($row['TYPE']=='STUDENT'){
	//Create query
	$qry = "SELECT EMPLOYEE.FNAME AS FNAME,EMPLOYEE.LNAME AS LNAME,EMPLOYEE.COMPANY AS COMPANY,EMPLOYEE.POST AS POST,EMPLOYEE.EMAIL AS EMAIL
			FROM EMPLOYEE ,(SELECT *
                                         FROM STUDENT 
                                          WHERE EMAIL = "."'".$_SESSION['USER_ID']."'".") AS Y
			WHERE Y.COLLEGE = EMPLOYEE.UG
			OR Y.COLLEGE = EMPLOYEE.PG
			OR Y.COLLEGE = EMPLOYEE.ANY";
	//Execute query
	$result = mysqli_query($link,$qry);
	...
	echo '<div id="tec">
			<div style="height:30px;width:720px;background-color:skyblue;margin-left:10px"><h3>'.$row['FNAME']."      ".$row['LNAME'].'</h3></div>
			<p style="margin-left:50px">
			<ul><li> COMPANY: '.$row['COMPANY'].'</li>
